...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 2. Several of critical bug fixes.
 ...
 - `<AFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.
 ...
 @warning Do not instantiate a single `FMDatabase` object and use it across multiple threads. Instead, use `<AFMDatabaseQueue>`.

 */

@interface AFMDatabase : NSObject  {
...
/// @name new methods to fix bugs and be readable

/**
    Bind arguments to an array with known types, no more type-guessing!
*/
...
- (AFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
/// Cache queries before excecution and get parameter count
- (FMStatement*) cacheStatementForQuery:(NSString *)sql;

/// Remove queries no longer used - we don't call close that will be called by dealloc. @see also clearCachedStatements 
...
/** Amount of parameters for this sql-query */

@property (atomic, assign) int parameterCount;
