...
@implementation AFMResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(FMStatement *)statement usingParentDatabase:(AFMDatabase*)aDB
{
    AFMResultSet *rs = [[AFMResultSet alloc] init];
...
- (void)kvcMagic:(id)object
{
    ...
    for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
        ...
        const char *column = (const char *)sqlite3_column_text([_statement statement], columnIdx);
        
        // check for a null row
        if (column)
		{
            NSString *string = [NSString stringWithUTF8String:column];
            [object setValue:string forKey:[NSString stringWithUTF8String:sqlite3_column_name([_statement statement], columnIdx)]];
        }
    }
}
...
- (id)objectForColumnIndex:(int)columnIdx
{
    ...
    if (returnValue == nil)
    {
        //HERE WAS NSNULL. Why would you ever want to introduce another type of data like this? returnValue = [NSNull null];
    }
...
- (void)setParentDB:(AFMDatabase *)newDb {
    _parentDB = newDb;
}
